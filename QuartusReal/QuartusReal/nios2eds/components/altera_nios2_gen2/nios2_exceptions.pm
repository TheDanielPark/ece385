...
@EXPORT = qw(
    ...
    $bus_inst_fetch_error_exc
    $bus_data_read_error_exc
);
...
our $bus_inst_fetch_error_exc;
our $bus_data_read_error_exc;
...
our $BUS_INST_FETCH_ERROR_EXC_ID;
our $BUS_DATA_READ_ERROR_EXC_ID;
our $NUM_EXC_IDS;
...
convert_to_c
{
    ...
    format_hash_as_c_macros($constants, $h_lines, $prefix);
    ...
    push(@$h_lines, "} ${prefix_lc}ExcInfo;");
    push(@$h_lines, "");
    push(@$h_lines, "extern ${prefix_lc}ExcInfo ${prefix_lc}excInfo[${prefix}NUM_EXC_IDS];");
    push(@$h_lines, "#endif /* ALT_ASM_SRC */");

    push(@$c_lines, "");
    push(@$c_lines, "/* Exception information */");
    push(@$c_lines, "${prefix_lc}ExcInfo ${prefix_lc}excInfo[${prefix}NUM_EXC_IDS] = {");
...
create_exception_constants
{
    ...
    $constants{BUS_INST_FETCH_ERROR_EXC_ID} = 22;  # instruction bus fetch error
    $constants{BUS_DATA_READ_ERROR_EXC_ID} = 23;   # data bus read error
    $constants{ECC_ITLB_ERROR_EXC_ID} = 24;   # ECC error on itlb
    $constants{ECC_INST_ERROR_EXC_ID} = 25;   # ECC error on inst
    $constants{ECC_RF_ERROR_EXC_ID} = 26;   # ECC error on rf
    $constants{ECC_DTLB_ERROR_EXC_ID} = 27;   # ECC error on dtlb
    $constants{ECC_DATA_ERROR_EXC_ID} = 28;   # ECC error on data
    $constants{ECC_DCACHE_ASYNC_ERROR_EXC_ID} = 29;   # ECC error on data
    $constants{NUM_EXC_IDS} = 30;
    ...
    $constants{BUS_INST_FETCH_ERROR_CAUSE_CODE} = 23;  # instruction bus fetch error
    $constants{BUS_DATA_READ_ERROR_CAUSE_CODE} = 24;   # data bus read error
...
create_exceptions
{
    ...
    $bus_inst_fetch_error_exc = add_exception($exceptions, {
        name        => "bus_inst_fetch_error",
        id          => manditory_int($constants, 
                         "BUS_INST_FETCH_ERROR_EXC_ID"),
        priority    => 11,
        invalidates_inst_value  => 1,
        cause_code  => 
          manditory_int($constants, "BUS_INST_FETCH_ERROR_CAUSE_CODE"),
    });
    ...
    $bus_data_read_error_exc = add_exception($exceptions, {
        name        => "bus_data_read_error",
        id          => manditory_int($constants, 
                         "BUS_DATA_READ_ERROR_EXC_ID"),
        priority    => 20,
        cause_code  => 
          manditory_int($constants, "BUS_DATA_READ_ERROR_CAUSE_CODE"),
