...
convert_to_c
{
    ...
    format_hash_as_c_macros($elaborated_mpu_info->{mpu_constants}, $h_lines, $prefix);
...
make_mpu_regions
{
    ...
    my $match_cmp_stage     = $d ? ($impl eq "small") ? "D"                     : "E"                      : ($impl eq "small") ? "D" : "F";
    my $match_cmp_addr      = $d ? ($impl eq "small") ? "D_mem_baddr_for_dmpu"  : "E_mem_baddr_for_dmpu"   : ($impl eq "small") ? "D_pc" : "F_pc";
    my $mpu_match_cmp_addr  = $d ? ($impl eq "small") ? "D_mem_baddr_dmpu"      : "E_mem_baddr_dmpu"       : ($impl eq "small") ? "D_pc_impu" : "F_pc_impu";
    my $match_mux_stage     = $d ? ($impl eq "small") ? "E"                     : "M"                      : ($impl eq "small") ? "E" : "F";
    ...
    for ($ri = 0; $ri < $num_regions; $ri++) {
        ...
        e_assign->adds(
          ...
          [["${rn}_wr_en", 1],
            "W_${m}_wr_operation & " .
            "(W_mpubase_reg_index[$index_ctrl_msb:$index_ctrl_lsb]
              == $ri)"],
        );

        e_register->adds(
          {out => ["${rn}_base", $region_base_sz], 
           in => "W_mpubase_reg_base[$base_ctrl_msb:$base_ctrl_lsb]",
           enable => "${rn}_wr_en"},
          ($mpu_use_limit ? 
              {out => ["${rn}_limit", $region_limit_sz], 
               in => "W_mpuacc_reg_limit[$limit_ctrl_msb:$limit_ctrl_lsb]",
               enable => "${rn}_wr_en"} :
              {out => ["${rn}_mask", $region_mask_sz], 
               in => "W_mpuacc_reg_mask[$mask_ctrl_msb:$mask_ctrl_lsb]",
               enable => "${rn}_wr_en"}),
          $d ? {out => ["${rn}_mt", 2],                  
               in => "W_mpuacc_reg_mt",
               enable => "${rn}_wr_en"} : (),
          ...
          {out => ["${rn}_perm", $region_perm_sz], 
           in => "W_mpuacc_reg_perm[$perm_ctrl_msb:$perm_ctrl_lsb]",
           enable => "${rn}_wr_en"},
        );
        ...
        push(@rd_mt_mux, $ri => "${rn}_mt");
        push(@rd_s_mux, $ri => "${rn}_s");
        ...
        push(@region_wave_signals,
          ...
          $d ? { radix => "x", signal => "${rn}_mt" } : "",
          $d ? { radix => "x", signal => "${rn}_s" } : "",
    ...
    my @region_read_wave_signals = (
      { divider => "$M Read" },
      { radix => "x", signal => "W_mpubase_reg_index" },
    ...
    my @region_mt_mux_table;
    my @region_perm_mux_table;
    my @region_s_mux_table;
    ...
    for ($ri = 0; $ri < $num_regions; $ri++) {
        ...
        push(@region_mt_mux_table,    $sel => "${rn}_mt");
        push(@region_perm_mux_table,  $sel => "${rn}_perm");
        push(@region_s_mux_table,     $sel => "${rn}_s");
    }


    e_mux->adds(
      $d ? { lhs => ["${match_mux_stage}_${m}_mt", 2, 0, $force_never_export],
        type => "priority", table => \@region_mt_mux_table } : (),
      $d ? { lhs => ["${match_mux_stage}_${m}_s", 2, 0, $force_never_export],
        type => "priority", table => \@region_s_mux_table } : (),
    ...
    my @region_lookup_wave_signals = (
      ...
      $d ? { radix => "x", signal => "${match_mux_stage}_${m}_mt" } : "",
      $d ? { radix => "x", signal => "${match_mux_stage}_${m}_s" } : "",
...
create_mpu_constants
{
    ...
    $constants{mpu_min_region_size_log2} = 8;
 ...
 */
$define ${prefix}MPUACC_REG_MASK_LSB (${prefix}MPUBASE_REG_BASE_LSB)
$define ${prefix}MPUACC_REG_MASK_MSB (${prefix}MPUBASE_REG_BASE_MSB)
$define ${prefix}MPUACC_REG_MASK_SZ (${prefix}MPUBASE_REG_BASE_SZ)
$define ${prefix}MPUACC_REG_MASK_UNSHIFTED_MASK (${prefix}MPUBASE_REG_BASE_UNSHIFTED_MASK)
$define ${prefix}MPUACC_REG_MASK_SHIFTED_MASK (${prefix}MPUBASE_REG_BASE_SHIFTED_MASK)
$define ${prefix}MPUACC_REG_MASK_MASK (${prefix}MPUBASE_REG_BASE_MASK)
$define SET_${prefix}MPUACC_REG_MASK(Reg, Val) SET_${prefix}MPUBASE_REG_BASE(Reg, Val)
$define GET_${prefix}MPUACC_REG_MASK(Reg) GET_${prefix}MPUBASE_REG_BASE(Reg)
EOM
